...
class CustomAttention(BaseAttention):
    ...
    def __init__(self, config):
        ...
        # representation). This is now the *only* signal deciding how much
        # weight a query puts on each strictly-earlier block and on its own
        # block, so the gate's job is purely an init-time safety valve:
        # zero at init means every allowed (self, earlier-block) pair
        # starts tied at equal logits regardless of what the untrained Q/K
        # projections currently compute, i.e. routing starts uniform. The
        # gate opens up as training shows content-based routing helps.
    ...
    def forward(self, X):
        ...
        global_scores = torch.einsum('bhild,bhjd->bhilj', Q, Kbar) * (1.0 / math.sqrt(head_dim))
        ...
        # unconditionally at weight 1, scored the same content-similarity
        # way. We can't reuse the pooled `summary` to build this block's
        # own content key -- it sums over every query in the block,
        ...
        # only reflects rows 0..l, so pooling K with it stays causal.
        Kbar_causal = torch.matmul(summary_causal, K)  # (B,H,G,L,d) causal pooled content key, own block
        self_scores = torch.einsum('bhild,bhild->bhil', Q, Kbar_causal) * (1.0 / math.sqrt(head_dim))
